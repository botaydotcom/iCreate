...
public class EventDBAdapter {
  ...
  //Insert a new event into the Event table
    public long createEvent(Item _event) {
      ...
      + ", " + EVENTDB_STARTTIME + ", " + EVENTDB_ENDTIME + ", "+ EVENTDB_ALERTTIME + ", " + EVENTDB_TYPE +  ")" + " VALUES ('" +
      _event.GetTitle() + "','" +
    ...
    public Cursor getEventById(long rowId) throws SQLException {
        ...
        this.mDb.query(true, DATABASE_TABLE_EVENT, new String[] { EVENTDB_KEY_ID, EVENTDB_TITLE, EVENTDB_DESCRIPTION, EVENTDB_LOCATION, EVENTDB_PRIORITY, EVENTDB_REPEAT, EVENTDB_CATEGORY, EVENTDB_COMPLETED, EVENTDB_COLOR, EVENTDB_STARTTIME, EVENTDB_ENDTIME, EVENTDB_ALERTTIME   }, EVENTDB_KEY_ID + "=" + rowId, null, null, null, null, null);
        if (mCursor != null) {
            mCursor.moveToFirst();
        }
        return mCursor;
    ...
    public Cursor getEventByDate(Date datetime) throws SQLException {
        ...
        this.mDb.query(true, DATABASE_TABLE_EVENT, new String[] { EVENTDB_KEY_ID, EVENTDB_TITLE, EVENTDB_DESCRIPTION, EVENTDB_LOCATION, EVENTDB_PRIORITY, EVENTDB_REPEAT, EVENTDB_CATEGORY, EVENTDB_COMPLETED, EVENTDB_COLOR, EVENTDB_STARTTIME, EVENTDB_ENDTIME, EVENTDB_ALERTTIME   }, EVENTDB_STARTTIME + " >= " + t + " AND " + EVENTDB_STARTTIME + " < " + thedayafter , null, null, null, null, null);
        if (mCursor != null) {
            mCursor.moveToFirst();
        }
        return mCursor;
