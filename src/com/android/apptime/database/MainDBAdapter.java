...
public class MainDBAdapter {
	...
	private static final String DATABASE_CREATE_MAP = "create table "
			+ DATABASE_TABLE_MAP + " (" + MAPDB_KEY_ID
			+ " integer primary key autoincrement, " + MAPDB_TITLE + " text, "
			+ MAPDB_HORIZONTAL + " text not null, " + MAPDB_VERTICAL
			+ " text not null, " + MAPDB_LONGITUDE + " text not null, "
			+ MAPDB_LATITUDE + " text not null, " + MAPDB_LINK + " text" + ");";

	// SQL COMMAND TO CREATE TASK TABLE
	private static final String DATABASE_CREATE_TASK = "create table "
			+ DATABASE_TABLE_TASK + " (" + TASKDB_KEY_ID
			+ " integer primary key autoincrement, " + TASKDB_TITLE + " text, "
			+ TASKDB_DESCRIPTION + " text, " + TASKDB_LOCATION + " text, "
			+ TASKDB_PRIORITY + " text, " + TASKDB_REPEAT + " text not null, "
			+ TASKDB_CATEGORY + " text, " + TASKDB_COMPLETED
			+ " text not null, " + TASKDB_COLOR + " integer not null, "
			+ TASKDB_DEADLINE + " integer not null, " + TASKDB_ALERTTIME
			+ " integer not null, " + TASKDB_TYPE + " text not null" + ");";

	// SQL COMMAND TO CREATE EVENT TABLE
	private static final String DATABASE_CREATE_EVENT = "create table "
			+ DATABASE_TABLE_EVENT + " (" + EVENTDB_KEY_ID
			+ " integer primary key autoincrement, " + EVENTDB_TITLE
			+ " text, " + EVENTDB_DESCRIPTION + " text, " + EVENTDB_LOCATION
			+ " text, " + EVENTDB_PRIORITY + " text, " + EVENTDB_REPEAT
			+ " text not null, " + EVENTDB_CATEGORY + " text, "
			+ EVENTDB_COMPLETED + " text not null, " + EVENTDB_COLOR
			+ " integer not null, " + EVENTDB_STARTTIME + " integer not null, "
			+ EVENTDB_ENDTIME + " integer not null, " + EVENTDB_ALERTTIME
			+ " integer not null," + EVENTDB_TYPE + " text not null" + ");";

	// SQL COMMAND TO CREATE ITEM TABLE
	private static final String DATABASE_CREATE_ITEM = "create table "
			+ DATABASE_TABLE_ITEM + " (" + ITEMDB_KEY_ID
			+ " integer primary key autoincrement, " + ITEMDB_KEY_TASK
			+ " integer not null, " + ITEMDB_KEY_EVENT + " integer not null,"
			+ " FOREIGN KEY (" + ITEMDB_KEY_TASK + ") REFERENCES "
			+ DATABASE_TABLE_TASK + " (" + TASKDB_KEY_ID + "), "
			+ " FOREIGN KEY (" + ITEMDB_KEY_EVENT + ") REFERENCES "
			+ DATABASE_TABLE_EVENT + " (" + EVENTDB_KEY_ID + "));";

	// SQL COMMAND TO CREATE CATEGORY TABLE
	private static final String DATABASE_CREATE_CATEGORY = "create table "
			+ DATABASE_TABLE_CATEGORY + " (" + CATEGORYDB_KEY_ID
			+ " integer primary key autoincrement, " + CATEGORYDB_TITLE
			+ " text not null);";

	// SQL COMMAND TO CREATE ALERTTYPE TABLE
	private static final String DATABASE_CREATE_ALERTTYPE = "create table "
			+ DATABASE_TABLE_ALERTTYPE + " (" + ALERTTYPEDB_KEY_ID
			+ " integer primary key autoincrement, " + ALERTTYPEDB_TITLE
			+ " text not null);";
